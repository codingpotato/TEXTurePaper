...
class Zero123:
    ...
    @torch.no_grad()
    def sample_model(self, input_img, x, y, z, sampler, precision='fp16',
                     h=256, w=256, ddim_steps=50, scale=3.0, ddim_eta=1.0):
        ...
        with precision_scope(self.device):
            with self.zero123.ema_scope():
                ...
                samples_ddim, _ = sampler.sample(S=ddim_steps,
                                                 ...
                                                 eta=ddim_eta,
                                                 x_T=None)
                image = self.zero123.decode_first_stage(samples_ddim)
                return torch.clamp((image + 1.0) / 2.0, min=0.0, max=1.0)

    def __call__(self, raw_img, x=0.0, y=0.0, z=0.0, h=256, w=256):
        input_img = self.preprocess_image(raw_img)
        transform = transforms.ToTensor()
        input_img = transform(input_img).unsqueeze(0).to(self.device)
        input_img = input_img * 2 - 1
        input_img = transforms.functional.resize(input_img, [h, w])

        sampler = DDIMSampler(self.zero123)
        return self.sample_model(input_img, x, y, z, sampler, h=h, w=w)
    ...
    def preprocess_image(self, input_img, preprocess=True):
        ...
        print(f'Preprocess_image took {time.time() - start_time:.3f}s.')
...
if __name__ == '__main__':
    ...
    image = zero123(raw_img=raw_img, x=-15, y=45)
    save_image(image, '../images/output.png')
