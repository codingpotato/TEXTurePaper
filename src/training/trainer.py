...
class TEXTure:
    ...
    def init_dataloaders(self) -> Dict[str, DataLoader]:
        init_train_dataloader = MultiviewDataset(
            self.cfg.render, device=self.device).dataloader()
    ...
    def init_logger(self):
        ...
        logger.add(lambda msg: tqdm.write(msg, end=""),
                   colorize=True, format=log_format)
        logger.add(self.exp_path / 'log.txt',
    ...
    def evaluate(self, dataloader: DataLoader, save_path: Path, save_as_video: bool = False):
        ...
        if save_as_video:
            ...
            def dump_vid(video, name): return imageio.mimsave(save_path / f"step_{self.paint_step:05d}_{name}.mp4", video,
                                                              fps=25,
    ...
    def full_eval(self, output_dir: Path = None):
        ...
        self.evaluate(self.dataloaders['val_large'],
                      output_dir, save_as_video=True)
        # except:
    ...
    def paint_viewpoint(self, data: Dict[str, Any]):
        ...
        logger.info(
            f'Painting from theta: {theta}, phi: {phi}, radius: {radius}')
        ...
        if self.cfg.guide.use_background_color:
            ...
            background = F.interpolate(self.back_im.unsqueeze(0),
                                       (self.cfg.render.train_grid_size,
                                        self.cfg.render.train_grid_size),
    ...
    def eval_render(self, data):
        ...
        z_normals = outputs['normals'][:, -1:, :, :].clamp(0, 1)
        # .permute(0, 2, 3, 1).contiguous().clamp(0, 1)
        rgb_render = outputs['image']
        ...
        rgb_render = rgb_render.permute(
            0, 2, 3, 1).contiguous().clamp(0, 1).detach()
        texture_rgb = outputs_with_median['texture_map'].permute(
            0, 2, 3, 1).contiguous().clamp(0, 1).detach()
        depth_render = outputs['depth'].permute(
            0, 2, 3, 1).contiguous().detach()
    ...
    def calculate_trimap(self, rgb_render_raw: torch.Tensor,
        ...
        update_mask[torch.bitwise_and(
            object_mask == 0, generate_mask == 0)] = 0
        ...
        if self.cfg.log.log_images:
            ...
            if self.paint_step > 1 or self.cfg.guide.initial_texture is not None:
                ...
                only_old_mask_for_vis = torch.bitwise_and(
                    refine_mask == 1, exact_generate_mask == 0).float().detach()
    ...
    def project_back(self, render_cache: Dict[str, Any], background: Any,
        ...
        if self.cfg.guide.strict_projection:
            ...
            z_was_better = z_normals + \
                self.cfg.guide.z_update_thr < z_normals_cache[:, :1, :, :]
            blurred_render_update_mask[z_was_better] = 0

        render_update_mask = blurred_render_update_mask
        self.log_train_image(
            rgb_output * render_update_mask, 'project_back_input')
        ...
        for _ in tqdm(range(200), desc='fitting mesh colors'):
            ...
            masked_pred = rgb_render.reshape(
                1, rgb_render.shape[1], -1)[:, :, mask > 0]
            masked_target = rgb_output.reshape(
                1, rgb_output.shape[1], -1)[:, :, mask > 0]
            ...
            masked_last_z_normals = z_normals_cache.reshape(1, z_normals_cache.shape[1], -1)[:, :,
                                                                                             current_z_mask == 1][:, :1]
            loss += (masked_current_z_normals -
                     masked_last_z_normals.detach()).pow(2).mean()
            loss.backward()
            optimizer.step()
    ...
    def log_diffusion_steps(self, intermediate_vis: List[Image.Image]):
        if len(intermediate_vis) > 0:
            step_folder = self.train_renders_path / \
                f'{self.paint_step:04d}_diffusion_steps'
